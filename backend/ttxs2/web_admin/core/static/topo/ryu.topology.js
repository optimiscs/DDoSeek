...
var elem = {
    force: d3.layout.force()
        ...
        .on("tick", _tick),
    svg: d3.select("#svgcontent").insert("svg")
        .attr("id", "topology")
        .attr("width", "100%")
...
function _dragstart(d) {
    ...
    d3.json("/core/stats/flow/" + dpid, function(e, data) {
        flows = data[dpid];
        console.log(flows);
...
elem.update = function () {
    ...
    nodeEnter.append("image")
        .attr("xlink:href", "/static/topo/router.svg")
    ...
    nodeEnter.append("text")
        ...
        .text(function(d) { return "Switch #" + trim_zero(d.dpid); });
...
function initialize_topology() {
    /*
    d3.json("localhost:8080/v1.0/topology/switches")
        .header("Access-Control-Allow-Origin", "*") 
        .get(function(error, switches) {
            d3.json("localhost:8080/v1.0/topology/links")
                .header("Access-Control-Allow-Origin", "*")
                .get(function(error, links) {
                    topo.initialize({switches: switches, links: links});
                    elem.update();
        });
    });
    */
    d3.json("/core/v1.0/topology/switches", function(error, switches) {
        d3.json("/core/v1.0/topology/links", function(error, links) {
            topo.initialize({switches: switches, links: links});
            elem.update();
        });
    });
